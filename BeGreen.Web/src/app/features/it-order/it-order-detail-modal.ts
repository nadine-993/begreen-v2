...
@Component({
    selector: 'app-it-order-detail-modal',
    standalone: true,
        ...
        <div class="modal-body">
            ...
            <div class="close-action" *ngIf="canClose()">
                <div class="form-group">
                    ...
                    <textarea [(ngModel)]="closeNote" placeholder="Explain the fix or resolution..."></textarea>
                </div>
                <button class="btn-close-order" (click)="closeOrder()" [disabled]="isSubmitting">
                    {{ isSubmitting ? 'Closing...' : 'Mark as Fixed' }}
                </button>
            </div>
        </div>
  ...
  `,
    ...
    .btn-close-order:hover { filter: brightness(1.1); }
...
export class ItOrderDetailModalComponent implements OnInit {
    @Input() request: any;
    ...
    canClose(): boolean {
        ...
        // Rule: Only Information Technology department can close
        return this.currentUser.department?.toLowerCase() === 'information technology';
    }
    ...
    closeOrder() {
        ...
        this.itService.closeOrder(this.request.id, this.closeNote).subscribe({
            next: (updated) => {
