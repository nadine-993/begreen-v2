...
@Component({
  selector: 'app-cash-advance-detail-modal',
  standalone: true,
  ...
  styles: [`
    ...
    .modal-content { background: #ffffff; border-radius: 32px; width: 90%; max-width: 800px; max-height: 85vh; display: flex; flex-direction: column; overflow: hidden; box-shadow: 0 50px 100px -20px rgba(0,0,0,0.25); }
    ...
    .section h3 { font-size: 1.1rem; color: var(--text-dark); margin-bottom: 16px; font-weight: 700; border-left: 4px solid var(--primary); padding-left: 12px; }
    .description-text { background: #f8fafc; padding: 16px; border-radius: 12px; color: #334155; line-height: 1.6; }
    ...
    .timeline-note { margin: 8px 0 0 0; background: #f8fafc; padding: 10px 14px; border-radius: 12px; font-size: 0.85rem; color: #475569; border-left: 3px solid #cbd5e1; }
...
export class CashAdvanceDetailModalComponent {
  @Input() request: any;
  ...
  approve() {
    ...
    this.cashAdvanceService.approveRequest(this.request.id, this.approvalNote || undefined).subscribe({
      next: (res) => {
  ...
  reject() {
    ...
    this.cashAdvanceService.rejectRequest(this.request.id, this.approvalNote || undefined).subscribe({
      next: (res) => {
        this.isSubmitting = false;
        this.request = res;
        this.success.emit();
  ...
  downloadPdf() {
    ...
    this.pdfService.downloadCashAdvancePdf(this.request).subscribe({
      next: () => {
